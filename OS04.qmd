...
The function `bmi()` has an Error. 
The argument hgt is receiving data that is not positive and we request that users double check that their data is in correct before inputing into the `bmi()` function.

## admiral Example

The admiral R package began in earnest in 2021. 
A core desing principal was to have informative messaging to our users making use of the package.
...
This adoption had the added benefit of adopting snapshot testing, which we will discuss in the last section.

The admiral R package has a helper function admiraldev that is only utilized by admiral developers. 
...
Below we have the `assert_s3_class` which checks if the class of the argument is of the S3 class - think dataframe/tibble.
...
Most of the `assert_` functions within admiraldev follow similar code pattern, i.e. checking an arugment and if an issue/error arises providing a succinct yet informative message. 
We encourage readers to check out the `assert_` functions in admiraldev GitHub Repo for additional examples of using cli and glue R packages.

## Quality of Life Improvements and Conclusion

snapshot testing - something

standalone scripts - something

tie it all together and ship it amazon express
