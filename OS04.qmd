...
* TODO: Daniel to add/discuss `arg_name = rlang::caller_arg(x)` and what it does
* TODO: Daniel to add/discuss  `call = parent.frame()` and what it does
...
**Definition:** Snapshot testing involves capturing output (snapshots) from a function or process at a point in time and later checking to see if this output matches the expected value. This is particularly helpful for testing visual outputs, complex data structures, and any output that might be cumbersome to define manually beforehand.

**Implementation in R:** The testthat package provides snapshot testing capabilities, allowing you to capture current outputs and compare them against stored snapshots during tests. If outputs change unexpectedly, the differences are highlighted, prompting a review.
standalone scripts - something

### Standalone

**Standalone Nature:** The script is designed to be self-contained, meaning it can be copied and used in other R projects without having to depend on the entire rlang package. This is useful for developers who want to use certain type-checking utilities without introducing unnecessary dependencies.

No no need to re-write everything!

## CONCLUSIONS

In conclusion, the paper illustrates the critical role of clear and user-friendly messaging in enhancing software usability, particularly for new users navigating R programming. 
...
<https://cli.tidyverse.org/>
<https://pharmaverse.github.io/admiral>
<https://pharmaverse.github.io/admiraldev>
